...
if __name__ == "__main__":
  ...
  envs = jax.vmap(game_obj.step)(state=envs, action=action_3)
  envs = jax.vmap(game_obj.step)(state=envs, action=action_3)
  rewards = jax.vmap(game_obj.rewards)(state=envs)
  
  # Convert back to numpy to avoid numerical errors!
  mean_rewards = np.mean(np.array(rewards))
  assert(mean_rewards == 3.25)
